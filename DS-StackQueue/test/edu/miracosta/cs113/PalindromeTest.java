...
public class PalindromeTest {
    ...
    private boolean isPalindrome(String s) {
        ...
        for (String digit:
             splitString) {
            if (!digit.equals(" ")) {
                stack1.push(digit.toLowerCase());
            }
        }

        // stack length is odd
        if (stack1.topOfStack % 2 == 0) {
            while (stack1.topOfStack - 1 != stack2.topOfStack) {
        ...
        if (stack1.topOfStack % 2 == 1) {
            while (stack1.topOfStack != stack2.topOfStack) {
                stack2.push(stack1.pop());
            }
            while (!stack1.empty()) {
                if (!stack1.pop().equals(stack2.pop()))  {
